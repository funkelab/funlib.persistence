...
class KlbAdaptor:
    def __init__(self, filename, attr_filename=None):
        ...
        logger.debug("Found chunk_shape %s from header" % str(self.chunk_shape))
        ...
        if attr_filename is not None:
            ...
            attributes_file = os.path.join(os.path.split(filename)[0], attr_filename)

            if not os.path.isfile(attributes_file):
                raise IOError(
                    "no attributes file %s found next to %s" % (attr_filename, filename)
                )
            ...
            with open(attributes_file, "r") as f:
                ...
                if "resolution" in attributes:
                    ...
                    logger.debug(
                        "Overwriting voxel size with %s " % str(self.voxel_size)
                    )
                if "shape" in attributes:
                    self.shape = Coordinate(attributes["shape"])
                    logger.debug("Overwriting shape with %s " % str(self.shape))
                if "offset" in attributes:
                    offset = Coordinate(attributes["offset"])
                    logger.debug("Setting offset to %s " % str(offset))

        self.roi = Roi(offset, self.shape * self.voxel_size)
    ...
    def __getitem__(self, slices):
        if len(self.files) == 1:
            ...
            file_indices = range(slices[0].start, slices[0].stop)

            slices = slices[1:]

            return np.array(
                [self.__read_file(self.files[i], slices) for i in file_indices]
            )
    ...
    def __read_file(self, filename, slices):
        ...
        return pyklb.readroi(
            filename, tuple(s.start for s in slices), tuple(s.stop - 1 for s in slices)
        )
