...
class SharedGraphProvider(object):
    ...
    def read_blockwise(self, roi, block_size, num_workers):
        ...
        master = multiprocessing.Process(
            ...
            args=(self, roi, block_size, num_workers, block_queue),
        )
        master.start()
        ...
        while True:
            ...
            try:
                ...
                if block is None:
                    logger.debug("Found None in queue, returning")
                    break

                block_nodes, block_edges = block

                logger.debug("Read graph data from queue in %.3fs", time.time() - start)
        ...
        nodes = {k: np.concatenate(v) for k, v in nodes.items()}
        edges = {k: np.concatenate(v) for k, v in edges.items()}
...
def read_blockwise_master(graph_provider, roi, block_size, num_workers, block_queue):
    ...
    task = Task(
        ...
        process_function=lambda: read_blockwise_worker(graph_provider, block_queue),
        fit="shrink",
...
def read_block(graph_provider, block, block_queue):
    ...
    nodes = {k: np.array(v) for k, v in nodes.items()}
    edges = {k: np.array(v) for k, v in edges.items()}
